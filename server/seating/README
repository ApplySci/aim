Seating for mahjong tournaments with 16-172 players. For any given number of players, a seating arrangement has been generated for up to 15 hanchan without repeat meetups between players (where possible).
...
This dataset is built on some of the solutions in Alice Miller's Breakout Room Allocation schedule Tool (Miller et al, 2020). Each such solution avoids repeat meetups between any pair of players.

We have reordered the data so that repeat vists to specific tables (breakout rooms, in Miller's format) are minimised for each player. When we have taken a subset of the data, this has been done to maximise indirect meetups for pairs of players who do not meet directly. This indirect meetup criterion was developed by Lester (2021). An indirect meetup is a player who plays each of players A and B, for players A and B who do not play each other,
