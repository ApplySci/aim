...
def add_user_get():
    ...
    def is_valid_user_email(email):
        return not (email.startswith("firebase-adminsdk") or email.endswith("gserviceaccount.com"))

    # Prepare user lists
    db_user_emails = {user.email for user, _ in db_users if is_valid_user_email(user.email)}
    sheet_user_emails = {user['email'] for user in sheet_users if is_valid_user_email(user['email'])}
    ...
    users = [
        {"email": user.email, "role": role.value, "in_sheet": user.email in sheet_user_emails}
    ...
    current_user_data = next((user for user in users if user["email"] == current_user.email), None)
    ...
    if not is_admin:
        form.role.choices = [choice for choice in form.role.choices if choice[0] != "admin"]
...
def remove_user_access():
    ...
    if access:
        try:
            ...
            revoke_result = googlesheet.revoke_sheet_access(tournament.google_doc_id, email)
...
def fix_access():
    ...
    if action == "add_to_sheet":
        ...
        result = googlesheet.revoke_sheet_access(tournament.google_doc_id, email)
        if result == True:
            return jsonify({"success": True})
        else:
            return jsonify({"success": False, "error": result})
